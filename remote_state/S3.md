...
terraform {
  backend "s3" {
    ...
    key            = "<YOUR_KEY_PATH>"          # e.g., dev/service-name/terraform.tfstate
    region         = "<YOUR_REGION>"            # e.g., ap-south-1
    dynamodb_table = "<YOUR_DYNAMODB_TABLE>"    # e.g., terraform-lock
    encrypt        = true
  }
}
...
- <YOUR_BUCKET_NAME> → S3 bucket they want to store Terraform state in
- <YOUR_KEY_PATH> → path in the bucket for this project (like dev/infra/terraform.tfstate)
- <YOUR_REGION> → AWS region (e.g., ap-south-1)
- <YOUR_DYNAMODB_TABLE> → DynamoDB table for state locking
